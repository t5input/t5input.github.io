...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('keydown', function (event) {
        ...
        if (isChineseMode && event.code.startsWith('Digit') && event.key >= '1' && event.key <= '9') {
            event.preventDefault();
    ...
    // 創建切換 UIOJKL/JKLUIO 模式的按鈕
    const strokeModeButton = document.createElement('button');
    strokeModeButton.id = 'strokeModeIndicator';
    
    // 設置按鈕樣式
    strokeModeButton.style.position = 'fixed';
    strokeModeButton.style.bottom = '70px';
    strokeModeButton.style.right = '20px';
    strokeModeButton.style.padding = '8px 15px';
    strokeModeButton.style.backgroundColor = '#FF9800';
    strokeModeButton.style.color = 'white';
    strokeModeButton.style.border = 'none';
    strokeModeButton.style.borderRadius = '5px';
    strokeModeButton.style.cursor = 'pointer';
    strokeModeButton.style.zIndex = '1000';
    strokeModeButton.style.fontSize = '16px';
    strokeModeButton.style.boxShadow = '0 2px 5px rgba(0,0,0,0.2)';
    strokeModeButton.style.transition = 'all 0.3s ease';
    
    // 添加懸停效果
    strokeModeButton.addEventListener('mouseover', function() {
        this.style.transform = 'scale(1.05)';
        this.style.boxShadow = '0 4px 8px rgba(0,0,0,0.2)';
    });
    
    strokeModeButton.addEventListener('mouseout', function() {
        this.style.transform = 'scale(1)';
        this.style.boxShadow = '0 2px 5px rgba(0,0,0,0.2)';
    });
    ...
    document.body.appendChild(strokeModeButton);
    updateModeIndicator();
    updateStrokeModeIndicator();
...
function selectPhrase(phrase) {
    ...
    // 移除 phrase 的第一個字元
    const modifiedPhrase = phrase.substring(1);
    
    // 獲取當前游標位置
    const startPos = outputInput.selectionStart;
    const endPos = outputInput.selectionEnd;

    // 在游標位置插入修改後的字串
    outputInput.value = outputInput.value.substring(0, startPos) + 
                        modifiedPhrase + 
                        outputInput.value.substring(endPos);

    // 移動游標到插入字串的結尾
    outputInput.selectionStart = outputInput.selectionEnd = startPos + modifiedPhrase.length;
